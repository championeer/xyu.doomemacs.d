...
- [[#dired-配置][DIRED 配置]]
- [[#ivy-rich显示命令的详细描述][ivy-rich：显示命令的详细描述]]
- [[#find-file-in-project陈斌的插件用于在项目中快速找到文件][find-file-in-project：陈斌的插件，用于在项目中快速找到文件]]
- [[#eros][Eros]]
- [[#补全company配置][补全（Company）配置]]
- [[#yasnippets][YASnippets]]
- [[#smart-parentheses][Smart Parentheses]]
- [[#avy-配置一个强大的方便跳转等操作的包][Avy 配置：一个强大的方便跳转等操作的包]]
- [[#emojis][Emojis]]
- [[#拼音搜索配置pyimpyim-basedict][拼音搜索配置（Pyim/Pyim-basedict）]]
- [[#ligature配置][ligature配置]]
...
#+RESULTS:
| company-auctex-init | evil-normalize-keymaps | company-mode-set-explicitly |
